...
public class StudentController {
    ...
    // Demo Student ID
    private final String DEMO_STUDENT_ID = "S101";
    private final String DEMO_STUDENT_NAME = "Demo Student";
    ...
    @FXML private TableColumn<CheckedOutItem, String> checkedOutByColumn;  // NEW column
    ...
    public void initialize() {
        ...
        // NEW: Setup "Checked Out By" column
        if (checkedOutByColumn != null) {
            checkedOutByColumn.setCellValueFactory(cellData -> cellData.getValue().checkedOutByProperty());
        }
    ...
    // UPDATED: Process the checkout with student information
    private void processCheckout(BookItem item) {
        ...
        // Add to checked out items with due date AND student info
        LocalDate dueDate = LocalDate.now().plusDays(14);
        String userInfo = DEMO_STUDENT_NAME + " (" + DEMO_STUDENT_ID + ")";
        libraryData.addCheckedOutItem(item.getId(), item.getTitle(), dueDate, userInfo);
        
        // Update student's checkout count
        libraryData.updateMemberCheckoutCount(DEMO_STUDENT_ID, true);
        
        // Show success message
        showMessage("Successfully checked out: " + item.getTitle() + " for " + DEMO_STUDENT_NAME, true);
